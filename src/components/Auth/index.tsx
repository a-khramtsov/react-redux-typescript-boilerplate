...
const Login: FC = () => {
	...
	const handleSubmit = async (dataObj: LoginPayloadType, setSubmitting: (val: boolean) => void) => {
		setSubmitting(true);
			...
			>
				{({ values, setFieldValue, isSubmitting }) => (
					<Form className={styles.form}>
						...
						<LoginInput value={values.login} onChange={(e: OnChangeEvent) => setFieldValue('login', e.target.value)} />
						<LoginInput value={values.password} onChange={(e: OnChangeEvent) => setFieldValue('password', e.target.value)} />

						<button className={styles.button} disabled={isSubmitting}>Войти</button>
